...
class User extends CI_Controller 
{
    ...
    public function Add()
    {
        ...
        $this->form_validation->set_message('required','{filend} tidak boleh kosong');
        if($this->form_validation->run() == false)
        {
            ...
            if(!$this->upload->do_upload('galery')) {
                $this->session->set_flashdata('pesan', 'Data gagal diupload.!');
    ...
    public function Ubah($id = null)
    {
        ...
        if ($this->form_validation->run()== FALSE)
        {
            ...
            if(!$this->upload->do_upload('galery')) {
                ...
                $this->session->set_flashdata('pesan', '<div class="alert alert-success">Data berhasil diperbaharui.!');
                redirect('Admin/User','refresh');
            } else {
                // jika diubah tanpa gambar
                $passwordhash=password_hash($this->input->post('password'),PASSWORD_DEFAULT);
